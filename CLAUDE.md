...
### Rollout em produção — CONCLUÍDO (07/07/2026)

Executado via terminal web da Hostinger (nunca SSH direto — chave local não autorizada no servidor). Passos reais:

1. **Backup** (`pg_dump -F c`) do banco `oficina` dentro do container `oficina-eletrica-postgres` antes de qualquer alteração.
2. **Transferência do SQL via base64**: o terminal web do Hostinger faz **tradução automática do texto exibido** (BEGIN→INÍCIO, END→FIM, `true`→`verdadeiro`, `updated_at`→`atualizado_em`, `DO $$`→`FAÇA $$$` com 3 cifrões — quebra a sintaxe de dollar-quoting) — **isso não é só cosmético, afeta o arquivo real gravado no disco**, diferente do gotcha já documentado no projeto irmão (que era só exibição). Correção: codificar o conteúdo em base64 (sem palavras traduzíveis) antes de colar, decodificar no servidor, e **sempre validar com `sha256sum` comparando local vs. servidor** antes de executar qualquer SQL/script colado assim.
3. Migration `007_multi_tenant.sql` aplicada com sucesso — contagens de linha antes/depois idênticas (usuários:1, clientes:0, veículos:0, agendamentos:0, postos_trabalho:2, ordens_serviço:0), `oficinas` com 1 linha (`nome="Minha Oficina"`, `segmento="integrado"`, dados reais preservados, não resetados).
4. `docker compose build --no-cache` + `docker compose up -d` — build confirmado com o código novo via `grep -c resolverOficinaAtual`/`grep -c super-admin` no `dist/` compilado dentro da imagem (não confiar só no log "Built"). Logs pós-deploy mostraram `Módulo registrado: /super-admin` e `[jobs] ... ativa por loja`.
5. **`scripts/seed.js` não vem na imagem de produção** (Dockerfile só copia `dist/`) — copiado pro container via terminal (mesma técnica base64 + `sha256sum` do passo 2, usando heredoc `cat > arquivo << 'EOF'` em vez de uma única linha gigante — uma string base64 única muito longa em `echo '...' | base64 -d` truncou/quebrou o comando no terminal web mais de uma vez; heredoc multi-linha funcionou de primeira). Rodado com `node seed.js` dentro do container — criou o `super_admin` com sucesso, sem duplicar o admin/postos já existentes (idempotente).
6. **Smoke test em produção, confirmado via navegador**: login `superadmin@oficina.com` em `https://oficina-api.srv1747711.hstgr.cloud/super-admin/login` **funcionou de verdade**, painel mostra "Minha Oficina" (Oficina Mecânica + Auto-Elétrica, Ativa, 0 clientes/agendamentos — dados reais de produção, não os de teste local).
7. `configuracao_oficina` permanece inerte no banco de produção (não apagada) — migration de limpeza `008` fica para depois de alguns dias estável.

**Nota sobre ações em produção via agente**: cada escrita direta no container de produção (INSERT manual, gravar arquivo no container) foi bloqueada pelo classificador de segurança até confirmação explícita e específica do usuário para aquela ação exata — isso é o comportamento esperado/desejado, não um bug.

---

## Frontend Next.js real + API JSON (08/07/2026)

**Contexto**: o link do hub `/demo` do food-system-Sas-ERP apontava pro projeto Vercel `sistema-oficina-eletrica-erp.vercel.app`, que só tinha um `index.html` estático "Em construção" (`frontend/index.html`, commit `f33a759` — placeholder criado só pra reservar o nome do projeto). O painel real SEMPRE existiu — é o server-rendered dentro do próprio backend Express (`/admin`, `/super-admin`), em `https://oficina-api.srv1747711.hstgr.cloud`. O trabalho desta sessão não substitui esse painel (que continua 100% intacto) — constrói uma **camada nova de frontend Next.js** que consome os mesmos dados via uma API JSON aditiva, para os módulos que já são reais.

**Descoberta importante**: dos 5 módulos citados no texto do placeholder ("cadastros, OS & estoque, PDV/financeiro, localização"), só `admin` (login/dashboard/agendamentos/manutenção/config) e `super-admin` (multi-loja) são de verdade funcionais. `cadastros`, `pdv-financeiro` e `localizacao` são só `GET /health` (11 linhas cada, zero lógica). `os-estoque` é esqueleto parcial (58 linhas, não grava no banco). Construir frontend pra esses 3 hoje seria construir UI pra endpoints que não existem — **escopo consciente**: o Next.js novo cobre só `admin` + `super-admin`.

**Backend — API JSON aditiva** (`src/modules/api/`, prefixo `/api`, zero mudança nas rotas HTML existentes):
- `src/core/auth.ts`: `exigirApiAdmin`/`exigirApiSuperAdmin` — variante de `exigirAdmin`/`exigirSuperAdmin` que lê `Authorization: Bearer <token>` (não cookie) e responde 401 JSON (não redirect). Coexiste com o guard de cookie original.
- `src/modules/api/auth.ts`: `POST /api/auth/login` (mesma regra de negócio do form HTML — inclui desambiguação de e-mail em 2+ lojas via HTTP 300 + `multiplasLojas[]`) e `POST /api/auth/super-admin-login`.
- `src/modules/api/admin.ts`: `GET /api/admin/dashboard`, `GET/PUT /api/admin/configuracao`, `GET/POST /api/admin/manutencao`, `GET /api/admin/notificacoes` + `/count`. Reaproveita os helpers já existentes (`obterConfiguracao`, `listarPlanos`, `registrarManutencao`, `listarProximas`, `contarNaoLidas`, `listar`, `marcarTodasLidas`) — nenhuma regra de negócio duplicada.
- `src/modules/api/super-admin.ts`: `GET/POST /api/super-admin/lojas`, `GET/PUT /api/super-admin/lojas/:id`, `POST /api/super-admin/lojas/:id/toggle-ativo`.
- `src/index.ts`: CORS manual (sem lib nova) escopado só a `/api/*`, sem `Access-Control-Allow-Credentials` (auth é Bearer, não cookie — não precisa e não deve ser credentialed). Origens permitidas via env `FRONTEND_ORIGINS` (fallback `http://localhost:3000,https://sistema-oficina-eletrica-erp.vercel.app`).

**Frontend** (`frontend/` — reescrito do zero, `index.html` removido): Next.js 16.2.9 + Tailwind 4 + Zustand + axios, mesmo padrão dos projetos irmãos (`food-system-Sas-ERP`, `saude-beleza-erp`). Tokens do Design System Contract (dark cinza-azul `--surface-0..3`/`--border-ui`/`--radius-card`/`--accent`), cores extraídas do `layout.ts` do painel HTML existente (`#0A0B0E`/`#15171c`/`#23262e`/accent azul `#3b82f6`) para manter a MESMA identidade visual do painel antigo. Auth via Bearer token em `localStorage` (não cookie) — `lib/api.ts` + `stores/auth.store.ts`, mesmo padrão dos projetos irmãos.

Páginas: `/login` (com fluxo de desambiguação multi-loja), `/dashboard`, `/manutencao`, `/configuracao`, `/notificacoes`, `/super-admin/login`, `/super-admin` (lista), `/super-admin/nova`, `/super-admin/[id]`.

**Bugs de ambiente descobertos e corrigidos**:
1. **Turbopack panic com `&` no nome da pasta**: `next dev`/build travava com `FileSystemPath("").join(...) leaves the filesystem root` — Turbopack não conseguia calcular path relativo por causa do `&` em "Sistema Oficina & Elétrica-ERP". Corrigido com `turbopack.root` explícito em `next.config.ts` (aponta pro próprio diretório `frontend/`, evita a auto-detecção de workspace via `package-lock.json` do diretório pai).
2. **Race real de login → dashboard**: `router.push("/dashboard")` logo após `setAuth()` podia servir uma árvore do **Router Cache do App Router** prefetchada ANTES do login (zustand ainda sem token) — bounce de volta pro `/login` mesmo com o token já salvo em localStorage. Corrigido trocando `router.push` por `window.location.href` (hard navigation) em `app/login/page.tsx` e `app/super-admin/login/page.tsx` — sidesteps o cache do router inteiramente.
3. **`frontend/.gitignore` estava quase vazio** (só `.vercel`) — teria commitado `node_modules/`, `.next/` e `.env.local` (secrets). Substituído por um `.gitignore` completo padrão Next.js.

**Validação — E2E real contra Postgres local** (não só `tsc`/`build`): aplicado `007_multi_tenant.sql` no banco `oficina_dev` local (estava desatualizado, só tinha 001-006). Testado via browser real (Chrome preview) contra o backend local (porta 3050): login admin, dashboard com KPIs reais, **registro de manutenção de verdade** (escreve no banco, aparece na lista), configuração (lê/salva), notificações (lista real), login super-admin, lista de lojas, criação de loja nova (+ admin), edição de loja, toggle ativo/inativo — todos os fluxos de escrita testados criaram uma loja de teste (`Oficina Teste E2E`) que foi apagada do banco local depois.

**Pendente desta sessão**: deploy em produção (VPS `oficina-api.srv1747711.hstgr.cloud` + Vercel `sistema-oficina-eletrica-erp`) — código commitado mas deploy ainda não executado até este ponto do CLAUDE.md ser escrito.
